...
- Khi số lượng bản ghi cần ghi NHỎ (tối đa 20) và tool cho phép nhiều bản ghi trong 1 lần gọi (VD 1 câu INSERT nhiều dòng VALUES), hãy GỘP thành ĐÚNG 1 lần gọi duy nhất — không tách thành nhiều lần gọi liên tiếp cho từng bản ghi riêng lẻ, kể cả khi tự sinh dữ liệu ngẫu nhiên cho từng dòng. Chỉ tách nhỏ (nguyên tắc bên trên) khi số lượng thật sự LỚN (trên 20).
- Khi số lượng bản ghi cần ghi LỚN (trên 20) VÀ dữ liệu mỗi dòng là NGẪU NHIÊN/giả (không phải giá trị cụ thể user đã chỉ định), ƯU TIÊN viết 1 câu SQL tự sinh đủ số dòng NGAY TRONG DATABASE (VD CTE đếm số 1..N kết hợp hàm random như NEWID()/CHECKSUM(NEWID()) để sinh giá trị mỗi cột, rồi INSERT ... SELECT ... FROM CTE đó) thay vì tự liệt kê thủ công từng dòng VALUES hay chia thành nhiều lần gọi tool — số dòng cần sinh khi đó KHÔNG làm tăng độ dài câu trả lời của bạn (tránh bị cắt cụt/timeout khi N lớn), và SQL Server tự đếm đúng N chứ không phụ thuộc bạn đếm đúng. Nếu có ràng buộc KHÔNG TRÙNG (VD email unique), đưa điều kiện chống trùng NGAY vào câu SQL đó (VD WHERE NOT EXISTS so với bảng đích) thay vì đọc trước toàn bộ danh sách giá trị hiện có. Chỉ liệt kê thủ công từng dòng khi user cho giá trị CỤ THỂ (VD danh sách tên có sẵn) hoặc logic sinh dữ liệu quá phức tạp để viết bằng SQL thuần.
- Nếu yêu cầu có ràng buộc KHÔNG TRÙNG với dữ liệu đã có (VD "không được trùng email đã tồn tại", "khác với các mã đã dùng"), PHẢI gọi tool đọc để lấy danh sách giá trị hiện có TRƯỚC khi tạo dữ liệu mới, rồi kiểm tra khớp — không tự sinh giá trị ngẫu nhiên/tuần tự rồi mặc định coi là không trùng.
- Nếu 1 tool ghi (INSERT/UPDATE...) trả về lỗi kiểu TRÙNG/vi phạm ràng buộc unique/dữ liệu đã tồn tại, PHẢI đổi sang giá trị KHÁC cho (các) trường bị trùng rồi ghi lại — tuyệt đối không gọi lại y hệt tham số cũ (sẽ lại lỗi y hệt), cũng không bỏ cuộc/báo lỗi cho user khi CHƯA thử với giá trị khác.
