...
import { ChannelResponse, ChannelMemberResponse } from './type/channel.response';
...
export class ChannelService {
  ...
  async addMember(dto: ChannelMemberDto): Promise<string> {
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      this.cachedService.invalidateList(CACHE.CHANNEL.TRACKERS.LIST_VERSION(channel.workspaceId, targetMemberId))
        .catch(err => this.logger.error(`Cache invalidation failed: ${err.message}`));
    });
  ...
  async addBatchMembers(dto: AddBatchMembersDto): Promise<string> {
    ...
    const users = await firstValueFrom(
      this.userClient.send(USER_MESSAGE_PATTERNS.GET_BATCH_USER_BY_IDS, { ids: targetMemberIds }),
    );
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const newMembers = newMemberIds.map(id => manager.create(ChannelMemberEntity, {
        channelId,
        memberId: id,
      }));
      ...
      await Promise.all(newMemberIds.map(id =>
        this.cachedService.invalidateList(CACHE.CHANNEL.TRACKERS.LIST_VERSION(channel.workspaceId, id))
          .catch(err => this.logger.error(`Cache invalidation failed for ${id}: ${err.message}`))
      ));
    });
  ...
  async removeMember(dto: ChannelMemberDto): Promise<string> {
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (channel.type === ChannelTypeEnum.DIRECT) {
        throw new RpcException(CHANNEL_ERROR.CANNOT_REMOVE_MEMBER_IN_DIRECT_CHANNEL);
      }

      // Hierarchy logic based on Workspace roles
      const performer = await this.checkWorkspacePermission(channel.workspaceId, performerId, [
        WorkspaceRoleEnum.OWNER,
        WorkspaceRoleEnum.ADMIN,
      ]);

      const target = await this.getWorkspaceMember(channel.workspaceId, targetMemberId);

      if (target.role === WorkspaceRoleEnum.OWNER) {
        throw new RpcException(CHANNEL_ERROR.CANNOT_REMOVE_OWNER);
      }

      if (performer.role === WorkspaceRoleEnum.ADMIN && target.role === WorkspaceRoleEnum.ADMIN) {
  ...
  private mapMemberToResponse(member: ChannelMemberEntity, user: MemberType): ChannelMemberResponse {
    return {
      id: member.id,
      email: user.email,
  ...
  async leaveChannel(channelId: string, memberId: string): Promise<string> {
    return await this.dataSource.transaction(async (manager) => {
      ...
      this.cachedService.invalidateList(CACHE.CHANNEL.TRACKERS.LIST_VERSION(channel.workspaceId, memberId))
        .catch(err => this.logger.error(`Cache invalidation failed: ${err.message}`));

      this.logger.log('User left channel: ', JSON.stringify({ channelId, memberId }, null, 2));
