...
import { withSerializableRetry, IOffsetResponse } from '@slack/common';
...
export class WorkShiftService {
  ...
  async bulkRegisterShifts(dto: BulkRegisterWorkShiftDto) {
    ...
    try {
      ...
      const holidaysMap = await this.holidayService.checkIfDatesAreHolidays(
        workspaceId,
        datesToCheck,
      );
      ...
      await this.policyService.checkLockDeadline(
        workspaceId,
        targetMember.role,
        userId,
        validationPayload.map((s) => s.workDate),
      );
      ...
      const insertedShifts = await withSerializableRetry(() =>
        this.dataSource.transaction('SERIALIZABLE', async (manager) => {
          ...
          return manager.find(WorkShiftEntity, {
      ...
      if (insertedShifts.length > 0) {
        ...
        // Fire and forget
        this.queueService
          .addBulkJobs(EQueueName.INTEGRATION_SYNC_QUEUE, syncJobs)
          .catch((err) => {
  ...
  async getWorkShifts(
    dto: GetWorkShiftsDto,
  ): Promise<IOffsetResponse<WorkShiftResponseDto[]>> {
    ...
    try {
      ...
      if (
        !this.calendarCommonService.isPrivileged(requestor.role) &&
        userId &&
        userId !== requestorId
      ) {
        throw new RpcException({
      ...
      const effectiveUserId = this.calendarCommonService.isPrivileged(
        requestor.role,
      )
        ? userId
        : requestorId;

      const [shifts, total] = await this.workShiftRepository.findAndCount({
        where: {
          workspaceId,
      ...
      const latestLogs: Array<{
        ...
        ? await this.attendanceLogRepository
            .createQueryBuilder('log')
            .distinctOn(['log.workShiftId'])
            .select('log.workShiftId', 'workShiftId')
            .addSelect('log.logType', 'logType')
            .where('log.workShiftId IN (:...shiftIds)', { shiftIds })
            .orderBy('log.workShiftId', 'ASC')
            .addOrderBy('log.recordedAt', 'DESC')
            .getRawMany()
        : [];
      const latestLogMap = new Map(
        latestLogs.map((log) => [log.workShiftId, log]),
      );
      ...
      responseDtos.forEach((dto) => {
        const latestLog = latestLogMap.get(dto.id);
        dto.hasAttendanceLogs = !!latestLog;
        dto.inOutStatus = !latestLog
          ? InOutStatus.NOT_STARTED
          : latestLog.logType === AttendanceLogType.CHECK_IN
            ? InOutStatus.IN
            : InOutStatus.OUT;
        const workedHours = reconciliationMap.get(
          `${dto.userId}:${dto.workDate}`,
        );
        if (workedHours !== undefined && workedHours > 0)
          dto.actualWorkHours = workedHours;
      });
      ...
      } as unknown as IOffsetResponse<WorkShiftResponseDto[]>;
    } catch (error) {
      if (error instanceof RpcException) throw error;
  ...
  async getWorkShiftDetail(dto: GetWorkShiftDetailDto) {
    const { id, workspaceId, requestorId } = dto;
    ...
    try {
      ...
      const response = plainToInstance(WorkShiftResponseDto, shift);
      const latestLog = [...(shift.attendanceLogs ?? [])].sort(
        (left, right) => right.recordedAt.getTime() - left.recordedAt.getTime(),
      )[0];
      response.hasAttendanceLogs = !!latestLog;
      response.inOutStatus = !latestLog
        ? InOutStatus.NOT_STARTED
        : latestLog.logType === AttendanceLogType.CHECK_IN
          ? InOutStatus.IN
          : InOutStatus.OUT;
      ...
      const reconciliation = await this.reconciliationRepository.findOne({
        ...
        select: ['actualWorkHours'],
      });
      if (reconciliation?.actualWorkHours !== undefined) {
        response.actualWorkHours = reconciliation.actualWorkHours;
      }

      return response;
    } catch (error) {
      if (error instanceof RpcException) throw error;
      this.logger.error('Error fetching work shift detail:', error);
      throw new RpcException({
        statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
        ...CALENDAR_ERROR.FETCH_SHIFTS_FAILED,
      });
    ...
    try {
      ...
      const updatedShift = await withSerializableRetry(() =>
        this.dataSource.transaction('SERIALIZABLE', async (manager) => {
          ...
          const updated = await manager.findOne(WorkShiftEntity, {
            where: { id },
          });
          if (!updated) {
            throw new RpcException({
      ...
      if (updatedShift) {
        this.queueService
          ...
          .catch((err) =>
            this.logger.error('Failed to dispatch sync job', err),
          );
      }
  ...
  async deleteWorkShift(dto: DeleteWorkShiftDto) {
    ...
    try {
      ...
      await this.dataSource.transaction(async (manager) => {
        ...
        const hasAttendanceLogs = await manager.exists(AttendanceLogEntity, {
      ...
      this.queueService
        ...
        .catch((err) =>
          this.logger.error('Failed to dispatch delete sync job', err),
        );
