...
const mockStartChat = jest.fn().mockReturnValue({ sendMessage: mockChatSendMessage });
const mockGenerateContent = jest.fn();
const mockGetGenerativeModel = jest.fn().mockReturnValue({ startChat: mockStartChat, generateContent: mockGenerateContent });
...
describe('GeminiStrategy', () => {
  ...
  const createStrategy = async (): Promise<GeminiStrategy> => {
    const module: TestingModule = await Test.createTestingModule({ providers: [GeminiStrategy] }).compile();
  ...
  describe('when GEMINI_API_KEY is missing', () => {
    it('throws LLM_PROVIDER_NOT_CONFIGURED instead of calling the SDK', async () => {
      ...
      expect(() => unconfigured.startChat({ model: 'gemini-2.0-flash', systemInstruction: '', tools: [], history: [] })).toThrow(
        RpcException,
      );
      await expect(unconfigured.generateStructured({ model: 'gemini-2.0-flash', systemInstruction: '', prompt: '', schema: {} })).rejects.toThrow(
        RpcException,
      );
    });
  });
  ...
  describe('startChat / sendMessage', () => {
    it('converts the model response into the generic LlmTurnResult shape', async () => {
      mockChatSendMessage.mockResolvedValue({
        response: { text: () => 'Đây là câu trả lời', functionCalls: () => [{ name: 'get_schema', args: { table: 'Orders' } }] },
      });
      ...
      expect(result).toEqual({ text: 'Đây là câu trả lời', toolCalls: [{ name: 'get_schema', args: { table: 'Orders' } }] });
    ...
    it('passes opts.temperature through to generationConfig when provided (Step 7)', async () => {
      mockChatSendMessage.mockResolvedValue({ response: { text: () => 'ok', functionCalls: () => [] } });

      strategy.startChat({ model: 'gemini-2.0-flash', systemInstruction: '', tools: [], history: [], temperature: 0.2 });
    ...
    it('serializes tool results into Gemini functionResponse parts', async () => {
      ...
      const session = strategy.startChat({ model: 'gemini-2.0-flash', systemInstruction: '', tools: [], history: [] });
      await session.sendMessage([{ name: 'get_schema', content: '{"Orders":[]}' }]);

      expect(mockChatSendMessage).toHaveBeenCalledWith([
        { functionResponse: { name: 'get_schema', response: { content: '{"Orders":[]}' } } },
      ]);
    });
    ...
    it('retries once on a transient 429 error, then succeeds', async () => {
      ...
      mockChatSendMessage
        ...
        .mockResolvedValueOnce({ response: { text: () => 'ok sau khi retry', functionCalls: () => [] } });
    ...
    it('does not retry a non-transient error', async () => {
      ...
      await expect(session.sendMessage('hi')).rejects.toThrow('invalid request');
  ...
  describe('generateStructured', () => {
    it('parses the JSON text response into the expected object', async () => {
      mockGenerateContent.mockResolvedValue({ response: { text: () => '{"action":"respond","answer":"chào"}' } });
    ...
    it('auto-injects format:"enum" for string enum fields (Gemini-specific requirement)', async () => {
      ...
      await strategy.generateStructured({
        ...
        systemInstruction: '',
        prompt: '',
        schema: { type: 'object', properties: { action: { type: 'string', enum: ['respond', 'delegate'] } } },
      });

      const passedSchema = mockGetGenerativeModel.mock.calls.at(-1)?.[0].generationConfig.responseSchema;
      expect(passedSchema.properties.action).toEqual({ type: 'string', enum: ['respond', 'delegate'], format: 'enum' });
    });
    ...
    it('retries on a transient 429/503 error just like startChat (Supervisor hits this path on every message)', async () => {
      ...
      mockGenerateContent
        ...
        .mockResolvedValueOnce({ response: { text: () => '{"action":"respond","answer":"ok sau retry"}' } });
